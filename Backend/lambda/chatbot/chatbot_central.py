...
def _handle_maritimo(user_query, conversation_history):
    ...
    # Compute per-item validation alerts + extract datos_completos
    alertas_respuesta = []
    datos_completos = None
    datos_extraidos = maritimo.extraer_datos_relevantes_maritimos(items)
    for datos_item in datos_extraidos:
        alertas_respuesta.extend(maritimo.validar_cotizacion(datos_item))

    # When response is XML, extract tarifa_id from it to find the full matching record.
    # This works even when multiple items were fetched (generacion_cotizacion picks best match).
    if respuesta_final.strip().startswith("<respuesta>"):
        ...
        # Fallback: if tarifa_id not found but only 1 item, use it directly
