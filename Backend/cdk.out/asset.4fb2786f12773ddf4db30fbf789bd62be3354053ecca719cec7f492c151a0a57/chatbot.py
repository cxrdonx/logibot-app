    ...
    try:
        ...
        if query_params:
            ...
            if destino:
                ...
                print("Haciendo scan completo (no recomendado)")
                response = table.scan()
                items = response.get('Items', [])
        ...
        if items:
            # OPTIMIZACIÓN: Solo mostrar resumen en lugar de JSON completo
            print(f"Muestra: {items[0].get('proveedor')} | {items[0].get('origen')} → {items[0].get('destino')}")
        print(f"{'='*60}\n")
...
def paso_3_generar_respuesta_natural(pregunta, items, peso_kg=None):
    """
    ...
    # LÍMITE: Enviar máximo 5 opciones al LLM para reducir tokens
    if len(datos_optimizados) > 5:
        print(f"⚠️ OPTIMIZACIÓN: Reduciendo de {len(datos_optimizados)} a 5 opciones para ahorrar tokens")
        datos_optimizados = datos_optimizados[:5]
    
    data_context = json.dumps(datos_optimizados, indent=2)
    
    # DEBUG optimizado: Solo mostrar resumen en lugar de JSON completo
    print(f"\n{'='*60}")
    print(f"DATOS ENVIADOS AL LLM:")
    print(f"{'='*60}")
    print(f"Registros: {len(datos_optimizados)}")
    print(f"Tokens estimados: ~{len(data_context)} caracteres")
    print(f"{'='*60}\n")
...
def lambda_handler(event, context):
    try:
        ...
        #OPTIMIZACIÓN: Extraer peso para filtrado inteligente
        ...
        # 4. INTELIGENCIA DE NEGOCIO (Data-to-Answer) - OPTIMIZADO
        respuesta_final = paso_3_generar_respuesta_natural(user_query, items, peso_kg)
