...
def paso_3_generar_respuesta_maritima(pregunta, items, conversation_history=None):
    ...
    Formatea directamente en Python para no superar el límite de tokens del LLM.
    Los datos completos se mantienen en memoria para validaciones de negocio.

    Returns:
        str: Respuesta con los campos requeridos por cotización encontrada.
    """
    if not items:
        return (
    ...
    # Construir respuesta directamente en Python (sin LLM) para respetar límite de tokens
    count = len(datos_optimizados)
    encabezado = (
        f"Encontré **{count} cotización**:" if count == 1
        else f"Encontré **{count} cotizaciones**:"
    )
    ...
    # Agregar alertas al pie si las hay
    ...
    print(f"\n{'='*60}")
    print(f"RESPUESTA MARITIMA FORMATEADA ({count} cotizaciones, sin llamada LLM adicional)")
    print(f"{'='*60}\n")
