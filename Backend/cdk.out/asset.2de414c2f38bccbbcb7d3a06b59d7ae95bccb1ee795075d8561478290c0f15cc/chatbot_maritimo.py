...
def paso_3_generar_respuesta_maritima(pregunta, items, clasificacion, conversation_history=None):
    ...
    if tipo == "generacion_cotizacion":
        ...
        respuesta = generar_xml_maritimo(enriched)
    ...
    if tipo == "busqueda_cuantitativa":
        ...
        if len(candidatos) == 1:
            ...
            return generar_xml_maritimo(candidatos[0])
...
def handler(event, context):
    ...
    try:
        ...
        # Include full tariff when exactly 1 result returned as XML
        if len(todos_datos) == 1 and respuesta_final.strip().startswith("<respuesta>"):
