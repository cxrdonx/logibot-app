...
def paso_3_generar_respuesta_maritima(pregunta, items, conversation_history=None):
    ...
    Ejecuta validaciones automáticas y presenta alertas si hay inconsistencias.

    Args:
        pregunta: Pregunta del usuario
        items: Resultados de DynamoDB (lista de cotizaciones)
        conversation_history: Historial de conversación

    Returns:
        str: Respuesta en formato Markdown con alertas de validación si aplica
    """
    if not items:
        return (
            "No encontré cotizaciones marítimas que coincidan con tu consulta en la base de datos.\n\n"
            "Puedes buscar por:\n"
    ...
    # Extraer datos relevantes y convertir tipos
    datos_optimizados = extraer_datos_relevantes_maritimos(items)

    # Ejecutar validaciones en todas las cotizaciones encontradas
    todas_las_alertas = []
    ...
    data_context = json.dumps(datos_optimizados, indent=2, ensure_ascii=False)
    alertas_context = (
        "\n\nALERTAS DE VALIDACION DETECTADAS:\n" + "\n".join(f"- {a}" for a in todas_las_alertas)
        if todas_las_alertas
        else "\n\nValidaciones: Sin alertas detectadas."
    )

    user_content = f"""
PREGUNTA DEL USUARIO: "{pregunta}"

COTIZACIONES MARITIMAS ENCONTRADAS EN LA BASE DE DATOS ({len(datos_optimizados)} cotizaciones):
{data_context}

{alertas_context}

Responde la pregunta del usuario de forma clara y profesional usando los datos anteriores.
Si hay alertas de validación, menciónalas de forma prominente.
Si hay transbordo (via_port presente), explícalo claramente.
Si el usuario pide verificar un total, usa la fórmula Total = Σ(Qi × Pi) + F_fijos.
No fuerces conversiones de moneda a menos que el usuario lo solicite explícitamente.
"""

    respuesta = invoke_nova_pro(_ALCE_SYSTEM_PROMPT, user_content, conversation_history)

    print(f"\n{'='*60}")
    print(f"RESPUESTA MARITIMA GENERADA ({len(datos_optimizados)} cotizaciones)")
    print(f"{'='*60}\n")
