...
def paso_0_clasificar_intencion(pregunta_usuario, conversation_history=None):
    ...
    1. COTIZACION (requiere_db=true) cuando:
       - Usuario pregunta por tarifas de un puerto/naviera NUEVA (no mencionada antes)
       - Usuario solicita información que NO está en el historial previo
       - Ejemplos: "¿Tarifas desde Rotterdam?", "¿Qué navieras van a Guatemala?", "¿Cuánto cuesta a Puerto Barrios?"
...
def generar_xml_maritimo(datos: dict) -> str:
    ...
    # Nota: vigencia de la tarifa
    dates = datos.get("dates", {})
    valid_till = dates.get("valid_till", "")
    nota = f"Válida hasta {valid_till}" if valid_till else ""
...
def paso_3_generar_respuesta_maritima(pregunta, items, conversation_history=None):
    ...
    if not items:
        return (
            "No encontré tarifas marítimas que coincidan con tu consulta.\n\n"
            "Puedes buscar por:\n"
    ...
    # CASO: múltiples tarifas → Markdown comparativo
    ...
    print(f"\n{'='*60}")
    print(f"RESPUESTA MARITIMA FORMATEADA ({count} tarifas, Markdown comparativo)")
    print(f"{'='*60}\n")
